...
interface QuestionCardProps {
  question: Question;
  onUpdateQuestion: (questionId: string, update: Partial<Question>) => Promise<void>;
  onDeleteQuestion: (questionId: string) => void;
}

const QuestionCard: React.FC<QuestionCardProps> = ({ question, onUpdateQuestion, onDeleteQuestion }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editedAnswer, setEditedAnswer] = useState(question.draftAnswer || '');
  const [editedQuestionText, setEditedQuestionText] = useState(question.question_text || question.text);
  ...
  useEffect(() => {
    ...
    setEditedQuestionText(question.question_text || question.text);
  ...
  const handleSave = async () => {
    try {
      ...
      // Update using the store's function
      await onUpdateQuestion(question.id, update);
  ...
  const handleDelete = async () => {
    if (window.confirm('Weet je zeker dat je deze vraag wilt verwijderen?')) {
      try {
        // Delete using the store's function
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="bg-white/50 dark:bg-slate-700/50 backdrop-blur-sm text-slate-700 dark:text-slate-300 text-xs px-2 py-1 rounded-full shadow-sm flex items-center">
                ...
                {question.videoTimestamp || question.timestamp || '00:00:00'}
              </span>
            </div>
